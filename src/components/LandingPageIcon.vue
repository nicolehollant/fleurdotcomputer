<template>
  <ClientOnly>
    ...
    >
      <Icon
        v-if="defs.iconIndex.current === 0"
        name="streamline:computer-chip-1"
        v-bind="$attrs"
        class="icon-animated"
      ></Icon>
      <Icon
        v-if="defs.iconIndex.current === 1"
        name="heroicons:computer-desktop"
        v-bind="$attrs"
        class="icon-animated"
      ></Icon>
      <Icon
        v-if="defs.iconIndex.current === 2"
        name="streamline:computer-desktop-favorite-heart-device-display-computer-favorite-like-heart-desktop"
        v-bind="$attrs"
        class="icon-animated"
      ></Icon>
      <Icon
        v-if="defs.iconIndex.current === 3"
        name="streamline:computer-mouse-computer-device-electronics-mouse"
        v-bind="$attrs"
        class="icon-animated"
      ></Icon>
      <Icon
        v-if="defs.iconIndex.current === 4"
        name="streamline:programming-bug-code-bug-security-programming-secure-computer"
        v-bind="$attrs"
        class="icon-animated"
      ></Icon>
      <Icon
        v-if="defs.iconIndex.current === 5"
        name="mdi:cake-variant-outline"
        v-bind="$attrs"
        class="icon-animated"
      ></Icon>
      <Icon v-if="defs.iconIndex.current === 6" name="mdi:flower" v-bind="$attrs" class="icon-animated"></Icon>
      <Icon v-if="defs.iconIndex.current === 7" name="mdi:flower-poppy" v-bind="$attrs" class="icon-animated"></Icon>
      <!-- <Icon v-if="defs.iconIndex.current === 0" name="mdi:food-outline" v-bind="$attrs" class="icon-animated"></Icon>
      <Icon v-if="defs.iconIndex.current === 1" name="mdi:cookie-outline" v-bind="$attrs" class="icon-animated"></Icon>
      <Icon
        v-if="defs.iconIndex.current === 2"
      ...
      <Icon
        ...
        class="icon-animated"
      ></Icon>
      <Icon v-if="defs.iconIndex.current === 4" name="mdi:coffee-outline" v-bind="$attrs" class="icon-animated"></Icon>
      <Icon
        v-if="defs.iconIndex.current === 5"
        name="mdi:cake-variant-outline"
        v-bind="$attrs"
        class="icon-animated"
      ></Icon>
      <Icon v-if="defs.iconIndex.current === 6" name="mdi:blender-outline" v-bind="$attrs" class="icon-animated"></Icon>
      <Icon v-if="defs.iconIndex.current === 7" name="mdi:stove" v-bind="$attrs" class="icon-animated"></Icon> -->
    </div>
  </ClientOnly>
</template>
...
function resetTransformations() {
  return {
    ...
    // color: ['#fb923c', '#f97316', '#ea580c', '#c2410c'][Math.floor(Math.random() * 4)],
