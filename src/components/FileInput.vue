<template>
  <div
    :class="{
      ...
      [cn('rounded-2xl overflow-hidden', classes?.wrapper)]: true,
    }"
    @dragover="dragover"
    @dragleave="dragleave"
    @drop="drop"
  >
    ...
    <label
      ...
      :class="
        cn(
          'group grid gap-4 rounded-2xl border-2 border-dashed bg-gradient-to-br p-8 transition duration-300 cursor-pointer',
          classes?.dropZone
        )
      "
    >
      <slot name="dropzone" :draggedOver="draggedOver">
        <div class="grid gap-2 text-center">
          ...
          <p>
            <span
              class="text-blue-800 dark:text-blue-300 transition duration-200 group-hover:text-blue-900 dark:group-hover:text-blue-400 group-hover:underline"
              >click</span
            >
      ...
      <slot name="files" :fnames="fnames" :removeFiles="removeFiles">
        <div class="flex flex-col gap-4 items-start pt-3 relative z-30" v-if="fnames?.length && !hideFiles">
          <ul class="p-2 rounded-lg bg-gray-200/40 dark:bg-gray-800/40 space-y-1">
          ...
          <button
            ...
            class="hover:text-orange-600 dark:hover:text-orange-400 flex gap-4 items-center bg-orange-100/40 dark:bg-orange-900/40 p-2 rounded-lg"
          >
            Remove All<Icon name="mdi:close"></Icon>
          </button>
        </div>
      </slot>
    </label>
...
<style lang="postcss">
.drop-zone-default {
  @apply bg-gradient-to-br transition duration-300 bg-gray-100 dark:bg-gray-900 from-background dark:from-gray-800;
}
.drop-zone-active {
  @apply bg-gradient-to-br transition duration-300 bg-blue-300 dark:bg-blue-900 from-background dark:from-gray-800;
}
</style>
